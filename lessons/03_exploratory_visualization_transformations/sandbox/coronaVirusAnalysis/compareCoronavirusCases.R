...
KenyaDat <- coronavirus %>% filter(country == "Kenya", province =="", type == "confirmed") %>% select(date,cases,type)
...
IndiaDat <- coronavirus %>% filter(country == "India", province =="", type == "confirmed") %>% select(date,cases,type)
...
nzDat <- coronavirus %>% filter(country == "New Zealand", province =="", type == "confirmed") %>% select(date,cases,type)
...
ggplot(nzDat, aes(nzDate, nzCases)) +  geom_line(color = "blue" ) + ylab("nz Cumulative confirmed cases") + ggtitle("Country: New Zealand")
